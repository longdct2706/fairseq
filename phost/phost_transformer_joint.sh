...
#SBATCH --output=phost_transformer-%A.out      # create a output file
#SBATCH --error=phost_transformer-%A.err       # create a error file
...
fairseq-train ${DATADIR} \
  ...
  --save-dir ${ASR_SAVE_DIR} --num-workers 20 --max-tokens 40000 --max-update 100000 \
  --task speech_to_text --criterion label_smoothed_cross_entropy --label-smoothing 0.1 --report-accuracy \
  --arch s2t_transformer_s --optimizer adam --lr 2e-3 --lr-scheduler inverse_sqrt \
  --warmup-updates 10000 --clip-norm 10.0 --seed 1 --update-freq 1 \
  --log-format simple --log-interval 100  --keep-last-epochs 15 \
