...
#SBATCH --output=multitask_augment-%A.out      # create a output file
#SBATCH --error=multitask_augment-%A.err       # create a error file
...
SAVE_DIR=${root_dir}/test_augment/multitask_augment_ckpt
PRETRAINED_DIR=${root_dir}/test_augment/pretrained_multitask
LOG_DIR=${root_dir}/test_augment/log/multitask_augm
...
fairseq-train ${DATADIR} \
  ...
  --task speech_text_joint_to_text_augment \
  ...
  --normalize-augm | tee ${LOG_DIR}/multitask_augment.train.log
...
fairseq-generate ${DATADIR} \
  --user-dir ./ \
  --config-yaml ${PRETRAINED_DIR}/config.yaml \
  --gen-subset dev_st --task speech_text_joint_to_text_augment --load-speech-only \
  --skip-invalid-size-inputs-valid-test \
  --infer-target-lang vi \
  --path ${SAVE_DIR}/${CHECKPOINT_FILENAME} --max-tokens 800000 --beam 5 \
  --scoring sacrebleu | tee ${LOG_DIR}/multitask_augment.dev.log

fairseq-generate ${DATADIR} \
  --user-dir ./ \
  --config-yaml ${PRETRAINED_DIR}/config.yaml \
  --gen-subset test_st --task speech_text_joint_to_text_augment --load-speech-only \
  --skip-invalid-size-inputs-valid-test \
  --infer-target-lang vi \
  --path ${SAVE_DIR}/${CHECKPOINT_FILENAME} --max-tokens 800000 --beam 5 \
  --scoring sacrebleu | tee ${LOG_DIR}/multitask_augment.test.log